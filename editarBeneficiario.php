...
if (isset($_GET['id'])) {
    ...
    $query = $conn->prepare("SELECT curp, nombres, paterno, materno, genero, fecha_nacimiento, estado_civil, discapacidad, aparato, poblacion, Id_Cat_Tip_Via, nombre_vial, numero_ext, letra_ext, numero_int, letra_int, nombre_asen, tipo_asen, referencia, celular, CVE_MUN, localidad, c_postal, clave_localidad, estado_nacimiento FROM beneficiario WHERE id_beneficiario=$id_beneficiario");
    $query->execute();
    $query->bind_result($curp, $nombre, $paterno, $materno, $genero, $nacimiento, $civil, $discapacidad, $aparato, $poblacion, $tipovia, $nomvia, $numext, $letraext, $numint, $letraint, $nomasen, $tipo_asen, $referencia, $celular, $municipio, $localidad, $cp, $cvelocalidad, $entidad);
    $query->store_result();

    $query1 = $conn->prepare("SELECT id_usuario, validado FROM beneficiario WHERE id_beneficiario = ?");
    ...
    <form id="FormEditarBeneficiario" class="ConfiguracionesUsuariosForm">
        ...
        <div class="FormData" style="width: 33.3%;">
            ...
            <select name="aparato" id="aparato">
                ...
                switch($aparato){
                    case "Ruedas":
                        echo '<option value="Ruedas" selected>Silla de ruedas</option>';
                        break;
                    case "PCI":
                        echo '<option value="PCI" selected>Silla de PCI</option>';
                        break;
                    case "PCA":
                        echo '<option value="PCA" selected>Silla de PCA</option>';
                        break;
                    case "Bath":
                        echo '<option value="Bath" selected>Silla de baño</option>';
                        break;
                    case "Andadera":
                        echo '<option value="Andadera" selected>Andadera</option>';
                        break;
                    case "Baston":
                        echo '<option value="Baston" selected>Baston</option>';
                        break;
                    case "Baston4":
                        echo '<option value="Baston4"  selected>Baston de 4 Puntos</option>';
                        break;
                    case "Auditivo":
                        echo '<option value="Auditivo" selected>Aparato Auditivo</option>';
                        break;
                    case "Muletas":
                        echo '<option value="Muletas" selected>Muletas</option>';
                        break;
                    case "Optometrico":
                        echo '<option value="Optometrico" selected>Optométrico</option>';
                        break;
                    case 'Diadema':
                        echo '<option value="Diadema" selected> Aparatos auditivos (Diadema)</option>';
                        break;
                    case 'Rinfantil':
                        echo '<option value="Rinfantil" selected>Silla de Ruedas (Infantil)</option>';
                        break;
                    case 'BInvidente':
                        echo '<option value="BInvidente" selected>Baston para Invidente</option>';
                        break;
                }
        ...
        <?php } else if ($_SESSION['rol'] == 2){  ?>
            ...
            $municipio=$_SESSION['clave_municipio'];
            ?>
            </select>
        </div>
        <?php } ?>
                ...
                success: function (response) {
                    ...
                    if (response === 'Success') {
                        <?php if($_SESSION['rol'] == 2){?>
                            WaitDoc('Beneficiario modificado', 'El beneficiario se modificó correctamente', 'redirect_to_listm');
                        <?php } else if($_SESSION['rol'] == 3){?>
                            WaitDoc('Beneficiario modificado', 'El beneficiario se modificó correctamente', 'redirect_to_lists');
                        <?php }else {?>
                            WaitDoc('Beneficiario modificado', 'El beneficiario se modificó correctamente', 'location.reload()');
                        <?php }?>
                    } else {
