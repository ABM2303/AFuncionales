...
<table id='tablaUsuarios'>
    <thead>
        <tr>
            ...
            <th>Supervisor que lo valido</th>
            <th>Estudios</th>
    ...
    <tbody>
        ...
        // $quienvalido = "";
            $conn = conectar();
            $query = $conn->prepare("SELECT id_beneficiario, curp, nombres, paterno, materno, aparato, id_supervisor, id_usuario, validado, comentario FROM beneficiario");
            $query->execute();
            $query->bind_result($id_benficiario, $curp, $nombre, $paterno, $materno, $aparato, $idvalidado, $idusuario,  $validado, $comentario);
            $query->store_result();
            while($query->fetch()){
                ...
                if($validado == "Si"){
                    ...
                    echo "<td>$func</td>";
                    $query1 = $conn->prepare("SELECT usuario FROM usuarios WHERE id = $idusuario");
                    $query1->execute();
                    $query1->bind_result($quienregistro);
                    $query1->store_result();
                    if($query1->fetch()){
                        echo "<td>$quienregistro</td>";
                    }
                    if($idvalidado != NULL){
                        $query2 = $conn->prepare("SELECT usuario FROM usuarios WHERE id = $idvalidado");
                        $query2->execute();
                        $query2->bind_result($quienvalido);
                        $query2->store_result();
                        if($query2->fetch()){
                            echo "<td>$quienvalido</td>";
                        }
                    }else{
                        echo "<td></td>";
                    }
                    ...
                    echo "<td><button data-tooltip='Eliminar registro' onclick=\"ResponseDelete($id_benficiario)\"><i class='bi bi-trash3'></i></button></td>";
                    echo "<td><a data-tooltip='Validar' onclick=\"ResponseValidar($id_benficiario,  $supervisor)\"><input type='checkbox'><span class='slider round'></span></a></td>";
                    if($comentario == NULL){
                        echo "<td><button data-tooltip='Comentar un error' onclick=\"ResponseError($id_benficiario)\"><i class='bi bi-x-circle'></i></button></td>";
                    }else{
                        echo <<<EOT
                        <td><button data-tooltip='Comentar un error' onclick='ResponseFull($id_benficiario, "$comentario")'><i class='bi bi-x-circle'></i></button></td>
                        EOT;
                    }
                    echo "</tr>";
                    }
