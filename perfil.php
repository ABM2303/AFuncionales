...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['nombre'], $_POST['apellido_paterno'], $_POST['apellido_materno'], $_POST['usuario'])) {
        ...
        $query->bind_param("sssss", $nombre, $apellido_paterno, $apellido_materno, $usuario, $usuario);
        if ($query->execute()) {
            $query->close();
    ...
    else if (isset($_POST['actual'], $_POST['nueva'], $_POST['repetir'])) {
        session_start();
...
<div id="Perfil">
    ...
    <div id="PerfilMenuDatos">
        ...
        <?php
        $query = $conn->prepare("SELECT u.nombres, u.apellido_paterno, u.apellido_materno, u.usuario, r.nombre 
                FROM usuarios u 
                INNER JOIN roles r ON u.id_rol = r.id_rol
                WHERE u.usuario = ?");
        $query->bind_param("s", $_SESSION['usuario']);
        $query->execute();
        $query->bind_result($nombre, $apellido_paterno, $apellido_materno, $usuario, $rol);
        $query->store_result();
        $query->fetch();
        $query->close();
        ?>
